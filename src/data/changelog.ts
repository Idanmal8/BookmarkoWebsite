...
export const changelog: ChangelogEntry[] = [
  {
    ...
    changes: [
      { tag: 'feature', text: 'Ginie now streams its replies as they\'re written, so you watch the answer form in real time instead of waiting for the whole thing to land.' },
      ...
      { tag: 'fix', text: 'The e-reader page no longer shifts underneath the "Ask Ginnie" sheet when it opens, so the text you\'re reading stays put.' },
  ...
  },
  {
    ...
    summary: 'Ginie is no longer just a book-recommender. Now it can act. Ask it to add a book and it drops a confirmation card right into the chat; tap Confirm and the book lands in your library with a little celebration. You\'re always in control, and nothing changes until you approve it.',
    changes: [
      { tag: 'feature', text: 'Ask Ginie to add books to your library. Say "add Dune", or "add the first two Lord of the Rings books", and it finds the right editions and offers them as tappable confirmation cards in the chat.' },
      ...
      { tag: 'improvement', text: 'A confirmed add plays out live with a book-cover card, a shelf that fills in, and a confetti moment, and your new book shows up in your Library right away.' },
  ...
  },
  {
    ...
    summary: 'Client-side changes only, bundling into the next app release. On a book\'s page, changing your reading status and updating your progress both got a refresh: the status picker now expands inline instead of popping up a menu, and you can type your exact page instead of fighting the slider.',
    changes: [
  ...
  },
  {
    ...
    summary: 'Client-side changes only, bundling into the next app release. The reader\'s visuals were refactored for a more immersive, modern feel: the controls now float as frosted-glass panels over the page, and opening a book reveals an already-settled page instead of the load jump.',
    changes: [
  ...
  },
  {
    ...
    date: '2026-06-15',
    title: 'Smoother loading with skeleton screens everywhere',
    summary: 'Client-side changes only, bundling into the next app release. Spinners are replaced with shimmer skeletons shaped like the real content, so screens feel faster and no longer jump around as data arrives. Each part of a screen now loads on its own, so the pieces that are ready show immediately instead of waiting for the slowest one.',
    changes: [
      ...
      { tag: 'improvement', text: 'Profile loads each section independently, so your avatar and background appear the moment they\'re ready without waiting on your reading goal, stats, or EXP, each with its own placeholder.' },
  ...
  },
  {
    ...
    changes: [
      ...
      { tag: 'feature', text: 'Display settings sheet: adjust font size, line spacing, font family (Book / Serif / Sans), and switch between Light, Sepia, and Dark themes, and your choices are remembered across books.' },
      ...
      { tag: 'feature', text: 'Translate any passage on-device: select text, choose from ten languages, and get an instant translation that works offline and for free, and no data leaves your phone.' },
      ...
      { tag: 'improvement', text: 'Backing out of the reader is instant again, because progress now syncs in the background instead of holding up the screen.' },
  ...
  },
  {
    ...
    summary: 'Client-side changes only, bundling into the next app release. Bring your whole Goodreads history into Bookmarko without leaving the app: sign in to Goodreads, press Export Library, and we sync everything automatically. CSV upload is kept as a manual fallback.',
    changes: [
      ...
      { tag: 'feature', text: 'In-app Goodreads sign-in via WebView: we detect the CSV download the moment you tap Export Library and pipe it straight into your library, with no copy/paste, no email round-trip, and your Goodreads cookies never leave the device.' },
      { tag: 'feature', text: 'Manual CSV upload fallback for users who already exported their library. It only accepts files named goodreads_* so you can\'t pick the wrong CSV by mistake.' },
  ...
  },
  {
    ...
    summary: 'Client-side changes only, bundling into the next app release. Many users tapped the (+) in the library header expecting to search for a book; that button only created collections. The library now has a single floating (+) menu that makes both actions obvious.',
    changes: [
  ...
  },
  {
    ...
    summary: 'Client-side changes only, bundling into the next app release. Deleting a book while inside a system status list (Reading, Finished, etc.) now prompts a confirmation that explains the action removes the book from your entire library, with an opt-out for users who clean up in bulk.',
    changes: [
      ...
      { tag: 'feature', text: 'New destructive confirmation dialog explains that status lists are filters, not folders, so confirming deletes the book from the whole library along with its progress and notes.' },
  ...
  },
  {
    ...
    summary: 'Client-side changes only, bundling into the next app release. Hebrew books missing a description in their primary provider now get one from NLI automatically, and archive builds can no longer accidentally point at sandbox.',
    changes: [
      { tag: 'improvement', text: 'Book details now ask the backend to cross-source a description by title. For example, a Google entry with no synopsis can pick up its Hebrew abstract from NLI.' },
      ...
      { tag: 'improvement', text: 'Debug and profile builds default to sandbox without needing an explicit dart-define flag, matching what developers actually want when running from VS Code or Xcode.' },
    ],
  },
  {
    version: 'Backend',
    ...
    changes: [
      ...
      { tag: 'improvement', text: 'Non-Hebrew searches are unchanged, and NLI is only consulted when it can actually help.' },
    ],
  },
  {
    version: '0.1.3+9',
